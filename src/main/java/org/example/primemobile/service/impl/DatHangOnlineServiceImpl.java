 ...
 * <ul>
 *   <li>Đơn online: {@code kenh_ban = "online"}, {@code trang_thai = "cho_xac_nhan"}.</li>
 ...
 *   <li>VNPay: {@code trangThaiThanhToan = "dang_chuyen_huong"} + {@code thoiGianHetHanTt = now + 15 phút}.</li>
...
public class DatHangOnlineServiceImpl implements IDatHangOnlineService {
    ...
    public DonHang taoDonHang(DatHangRequest request) {
        ...
        // ═══════════════════════════════════════════════════════════════════
        // BƯỚC 2 — KIỂM TRA KHO ONLINE & TÍNH TỔNG TIỀN HÀNG
        ...
        for (ChiTietGioHang item : danhSachGio) {
            ...
            // Giá snapshot: ưu tiên giá khuyến mãi
            BigDecimal donGia = (bienThe.getGiaKhuyenMai() != null)
                    ? bienThe.getGiaKhuyenMai()
                    : bienThe.getGiaBan();

            tongTienHang = tongTienHang.add(donGia.multiply(BigDecimal.valueOf(soLuongYeuCau)));

            log.debug("[DatHangOnline] SKU [{}] — soLuong={}, donGia={}, tonKho={}",
                    bienThe.getMaSku(), soLuongYeuCau, donGia, tonKhoHienTai);
        }
        ...
        for (ChiTietGioHang item : danhSachGio) {
            ...
            // Price snapshot: bất biến sau khi lưu dù giá sản phẩm sau đó thay đổi
            BigDecimal donGia = (bienThe.getGiaKhuyenMai() != null)
                    ? bienThe.getGiaKhuyenMai()
                    : bienThe.getGiaBan();
            ...
            log.debug("[DatHangOnline] Lưu CTDH — sku={}, soLuong={}, donGia={}",
                    bienThe.getMaSku(), item.getSoLuong(), donGia);
        }
    ...
    private KhachHang layKhachHang(Integer khachHangId, GioHang gioHang) {
        ...
        // Khách vãng lai hoàn toàn (chỉ có session) → bắt buộc phải đăng nhập để đặt hàng
        throw new IllegalArgumentException(
